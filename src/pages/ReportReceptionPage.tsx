...
import { ReportReceptionItemType } from "../data/type";
import { ReportReceptionDummyData } from "../data/dummy";
...
const ReportReceptionPage = () => {
  ...
  const [reportList, setReportList] = useState<ReportReceptionItemType[]>([]);
  const [currentReportList, setCurrentReportList] = useState<ReportReceptionItemType[]>([]);
  const [totalPage, setTotalPage] = useState(1);
  const [totalList, setTotalList] = useState(1);
  const [currentPage, setCurrentPage] = useState(1);
  const [currentList, setCurrentList] = useState(1);
  const [pageNumbers, setPageNumbers] = useState<number[]>([]);
  const [selectedReport, setSelectedReport] = useState<ReportReceptionItemType | null>(null);
  const [reportResult, setReportResult] = useState<string | null>(null);
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    setReportList(ReportReceptionDummyData);
  }, []);

  useEffect(() => {
    setHeaderProps({ level1Selected: "신고 관리", level2Selected: "신고접수" });
  }, [setHeaderProps]);
  ...
  return (
    <div className="w-full h-full">
      <TitleContainer level1="신고 관리" level2="신고접수" />
          ...
          <table className="w-full table-fixed">
            <thead className="pb-8 border-b-2 border-admin-gray-border">
              <tr>
                ...
                <th className="w-[10%]">콘텐츠ID</th>
                <th className="w-[20%]">사유</th>
                <th className="w-[20%]">신고일자</th>
                <th className="w-[10%]">상세보기</th>
              </tr>
            </thead>
            <tbody>
              {currentReportList.map((report, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <button
                      className="px-4 py-2 rounded-lg bg-admin-black text-white text-14 flex items-center justify-center pl-16 pr-12 border-none outline-none hover:outline-none active:outline-none focus:outline-none"
